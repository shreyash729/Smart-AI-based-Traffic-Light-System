...
def ml_model_timer(flow):
    ...
    if green_time_predict < 10:
        green_time_predict = 10
    elif green_time_predict > 40:
        green_time_predict = 40
...
signalTimerCoods = [(450, 480), (760, 160), (1020, 355), (760, 615)]
...
timeElapsed = 0
simulationTime = 300  # 0 for infinite simulation time
...
class Vehicle(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.direction == "right":
            ...
            if self.willTurn == 1:
                if self.lane == 2:
                    if self.crossed == 0 or self.x > stopLines[self.direction] - 180:
                        if (
                            self.x >= self.stop
...
class Main:
    ...
    while True:
        ...
        car_speed_text = font.render(
            "Current vehicle speed is 41.2 Km/h", True, black, white
        )
        ...
        for i in range(0, noOfSignals):
            ...
            vehicleCountTexts[i] = font.render(
                "Leg#" + str(i + 1) + " vehicle crossed Count : " + str(displayText),
                True,
                black,
                white,
            )
